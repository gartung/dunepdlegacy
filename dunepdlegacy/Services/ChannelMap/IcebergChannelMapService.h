...
//
// Implementation of hardware-offline channel mapping reading from files.  Keep the FELIX map functionality around just
// in case someone uses FELIX on Iceberg
// Separate files for TPC wires and SSP modules
///////////////////////////////////////////////////////////////////////////////////////////////////

#ifndef IcebergChannelMapService_H
#define IcebergChannelMapService_H
...
class dune::IcebergChannelMapService {
...
private:
  ...
  // hardcoded SSP channel map sizes -- left over from ProtoDUNE-SP
  ...
  // TPC Maps
  unsigned int farrayCsfcToOffline[1][3][4][128];  // implement as an array.  Do our own bounds checking

  // lookup tables as functions of offline channel number

  unsigned int fvAPAMap[1280]; // APA(=crate)
  unsigned int fvWIBMap[1280];	// WIB(slot)
  unsigned int fvFEMBMap[1280];	// FEMB(fiber)
  unsigned int fvFEMBChannelMap[1280];	// FEMB internal channel
  unsigned int fvStreamChannelMap[1280];	// RCE(FELIX) internal channel
  unsigned int fvSlotIdMap[1280]; // global WIB(slot) ID
  unsigned int fvFiberIdMap[1280]; // global FEMB(fiber) ID
  unsigned int fvChipMap[1280]; // Chip
  unsigned int fvChipChannelMap[1280]; //Chip internal channel 
  unsigned int fvASICMap[1280]; // ASIC
  unsigned int fvASICChannelMap[1280]; // ASIC internal channel
  unsigned int fvPlaneMap[1280]; // Plane type

  unsigned int fFELIXarrayCsfcToOffline[6][5][4][128];  // implement as an array.  Do our own bounds checking
  unsigned int fFELIXvAPAMap[1280]; // APA(=crate)
  unsigned int fFELIXvWIBMap[1280];	// WIB(slot)
  unsigned int fFELIXvFEMBMap[1280];	// FEMB(fiber)
  unsigned int fFELIXvFEMBChannelMap[1280];	// FEMB internal channel
  unsigned int fFELIXvStreamChannelMap[1280];	// RCE(FELIX) internal channel
  unsigned int fFELIXvSlotIdMap[1280]; // global WIB(slot) ID
  unsigned int fFELIXvFiberIdMap[1280]; // global FEMB(fiber) ID
  unsigned int fFELIXvChipMap[1280]; // Chip
  unsigned int fFELIXvChipChannelMap[1280]; //Chip internal channel 
  unsigned int fFELIXvASICMap[1280]; // ASIC
  unsigned int fFELIXvASICChannelMap[1280]; // ASIC internal channel
  unsigned int fFELIXvPlaneMap[1280]; // Plane type
  ...
  if (offlineChannel >= fNChans)
    {      
      throw cet::exception("IcebergChannelMapService") << "Offline TPC Channel Number out of range: " << offlineChannel << "\n"; 
    }
  };
  ...
  if (offlineChannel >= fNSSPChans)
    {      
      throw cet::exception("IcebergChannelMapService") << "Offline SSP Channel Number out of range: " << offlineChannel << "\n"; 
    }
  };
};

DECLARE_ART_SERVICE(dune::IcebergChannelMapService, LEGACY)
